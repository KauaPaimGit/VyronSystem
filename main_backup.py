...
# ============================================
# ROTAS: DOCUMENT RAG (Ingestão & Busca)
# ============================================

from app.brain_service import BrainService

@app.post("/brain/search", response_model=schemas.DocumentSearchResponse)
async def brain_search(request: schemas.DocumentSearchRequest, db: Session = Depends(get_db)):
    ...
    Utiliza embeddings + pgvector (distância de cosseno) para encontrar
    os 3 fragmentos mais relevantes para a query fornecida.

    - **query**: Texto de busca em linguagem natural
    - **limit**: Quantidade máxima de resultados (default: 3)
    - **filename**: (Opcional) Filtrar por nome de arquivo
    """
    try:
...
@app.post("/brain/upload", response_model=schemas.DocumentIngestResponse)
async def brain_upload(file: UploadFile = File(...), db: Session = Depends(get_db)):
    """
    Recebe um PDF via upload, processa e indexa no banco para uso pelo Agency Brain.
...
@app.post("/brain/ingest", response_model=schemas.DocumentIngestResponse)
async def brain_ingest(file_path: str, db: Session = Depends(get_db)):
    """
    Ingere um PDF local (caminho no servidor) no banco de dados.

    - **file_path**: Caminho absoluto do PDF na máquina

    Pipeline: PDF → extração de texto → chunking → embeddings → pgvector
    """
    try:
...
def brain_status(db: Session = Depends(get_db)):
    ...
    - total_chunks: Quantidade total de fragmentos indexados
    - total_files: Quantidade de arquivos processados
    - files: Lista com nome e quantidade de chunks por arquivo
    """
